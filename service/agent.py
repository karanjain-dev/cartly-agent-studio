...
class PersistentAgent:
    ...
    def reply(self, token, text, request_id, on_progress=None):
        ...
        with repo.connect() as conn:
            ...
            try:
                return self._reply(conn, s, token, text, request_id, on_progress or (lambda: None))
            finally:
                # Advisory locks also release automatically if the process dies.
                conn.rollback()
                conn.execute("SELECT pg_advisory_unlock(hashtext(%s))", ("agent:" + sid,))

    def _reply(self, conn, session, token, text, request_id, on_progress):
