...
export async function POST(req:Request){if(!sameOrigin(req))return jsonError('This request must come from the Cartly website.',403);if(!runtime().OPENAI_API_KEY)return jsonError('Live chat is awaiting the owner’s API connection. You can explore the workspace and policy.',503);
 ...
 try{const body:any=await req.json();if(typeof body.message!=='string'||!body.message.trim()||body.message.length>3000)return jsonError('Enter a message between 1 and 3,000 characters.');id=cookieId(req);if(!id)return jsonError('Please reload to start a session.',401);const row=await load(id);if(!row)return jsonError('Please reload to start a session.',401);const s=JSON.parse(row.payload);if(s.ended||s.turns>=20)return jsonError('Start a new conversation to continue.',409);
