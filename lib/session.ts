...
export type SupportRequest={kind:'return'|'cancel'|'coupon';orderId:string;itemId?:string;reason?:'change_of_mind'|'damaged'|'defective'|'wrong_item';unused?:boolean};
...
const demoUsers:Record<string,string>={return:'U018',cancel:'U014',delay:'U031'};
...
export function snapshot(s:Session,busy=false){const user=s.verified||demoUsers[s.demo]||null,orders=s.state.orders.filter((o:any)=>o.user_id===user);return{demo:s.demo,messages:s.messages,events:s.events,turns:s.turns,cost:s.cost,verifiedUser:user,orders,items:s.state.order_items.filter((item:any)=>orders.some((order:any)=>order.order_id===item.order_id)),changes:changes(freshWorld(),s.state),toolCalls:s.calls,model:s.model,ended:s.ended,busy,ready:!!runtime().OPENAI_API_KEY,date:s.state.config.today,proposal:s.proposal||null,notice:s.notice||''}}
