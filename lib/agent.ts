...
import {chatTools,chatInstructions} from './chat-contract';
import {agentTool,invalidateProposal,say} from './guarded-flow';
import {clone,changes} from './tools';
...
export async function runAgent(id:string,s:Session,message:string,emit:(e:any)=>void,signal:AbortSignal){
 ...
 let modelEvent:Event|null=null;
 invalidateProposal(s);
 ...
 activity('policy','Include the full policy',`Policy ${policy.version} and the reference date are included in this request.`,{policy_version:policy.version,prompt:'agent_v1.1 + hosted chat contract',today:s.state.config.today,timezone:'IST',source:'Full system prompt; this is not a search_policy tool call.'});
 await persist(id,s);
 try{for(let round=0;round<20;round++){
  ...
  const body={model:'gpt-6-astra',input:s.input,instructions:policy.prompt+chatInstructions,tools:chatTools,parallel_tool_calls:false,max_output_tokens:8192,reasoning:{effort:'high'},store:false,include:['reasoning.encrypted_content']};
  ...
  s.input.push(...(raw.output||[]));const calls:any[]=(raw.output||[]).filter((item:any)=>item.type==='function_call');let said=false;
  for(const item of raw.output||[]){if(!calls.length&&item.type==='message'){const text=(item.content||[]).filter((c:any)=>c.type==='output_text').map((c:any)=>c.text).join('');if(text){said=true;s.messages.push({role:'assistant',content:text});emit({type:'message',data:{role:'assistant',content:text}})}}}
  ...
  for(const call of calls){let args:any;try{args=JSON.parse(call.arguments)}catch{args={invalid_json:call.arguments}}const event=activity('tool',labels[call.name]||call.name,call.name,{arguments:args},'running');await persist(id,s);
   const before=clone(s.state),eventStart=s.events.length;
   const result=await agentTool(s,call.name,args),delta=changes(before,s.state);
   for(const added of s.events.slice(eventStart))emit({type:'activity',data:added});
   s.input.push({type:'function_call_output',call_id:call.call_id,output:JSON.stringify(result)});
   update(event,result.ok?'complete':'error',{arguments:args,...result});if(delta.length)activity('state','Save session changes',`${delta.length} change(s) applied to this demo session`,delta);
   if(result.ok&&call.name==='propose_action'&&result.result.proposal){
    say(s,'assistant',result.result.proposal.terms);
    emit({type:'message',data:s.messages.at(-1)});
    await persist(id,s);emit({type:'snapshot',data:snapshot(s)});return;
   }
